...
#new
            metric_name = f"{phase}/{var_name}/avg/monthly_rmse"
            self.log(metric_name, float(overall_rmse), **log_kwargs)
            metrics[metric_name] = float(overall_rmse)
...
#new
            metric_name = f"{phase}/{var_name}/time_mean_rmse"
            self.log(metric_name, float(time_mean_rmse), **log_kwargs)
            metrics[metric_name] = float(time_mean_rmse)
            ...
            #new
            metric_name = f"{phase}/{var_name}/time_stddev_mae"
            self.log(metric_name, float(time_std_mae), **log_kwargs)
            metrics[metric_name] = float(time_std_mae)
    ...
    def on_validation_epoch_end(self):
        ...
        if isinstance(self.logger, WandbLogger):
                    self.logger.experiment.log(self.val_metrics)
